...
def detailed_report_choose_months_ob_ch8(request):
    if 'username' in request.session:
        ...
        return render(request, 'branches/branch8/live_print_report/detailed_report_choose_months.html',context)
    return render(request, 'index.html')


def jan_details_live_ob_ch8(request):
    if 'username' in request.session:
...
def jan_print_live_ob_ch8(request):
    if 'username' in request.session:
...
def feb_details_live_ob_ch8(request):
    if 'username' in request.session:
...
def feb_print_live_ob_ch8(request):
    if 'username' in request.session:
        ag = pg1_new_beds.objects.all().order_by('roon_no')
        l = []
        t = []
...
def march_details_live_ob_ch8(request):
    if 'username' in request.session:
...
def march_print_live_ob_ch8(request):
    if 'username' in request.session:
        ag = pg1_new_beds.objects.all().order_by('roon_no')
        l = []
        t = []
...
def april_details_live_ob_ch8(request):
    if 'username' in request.session:
...
def april_print_live_ob_ch8(request):
    if 'username' in request.session:
        ag = pg1_new_beds.objects.all().order_by('roon_no')
        l = []
        t = []
...
def may_details_live_ob_ch8(request):
    if 'username' in request.session:
...
def may_print_live_ob_ch8(request):
    if 'username' in request.session:
        ag = pg1_new_beds.objects.all().order_by('roon_no')
        l = []
        t = []
...
def june_details_live_ob_ch8(request):
    if 'username' in request.session:
        ...
        return render(request, 'branches/branch8/live_print_report/live_monthly_details/june/june_details_live.html', context)
    return render(request, 'index.html')

def june_print_live_ob_ch8(request):
    if 'username' in request.session:
        ag = pg1_new_beds.objects.all().order_by('roon_no')
        l = []
        t = []
...
def july_details_live_ob_ch8(request):
    if 'username' in request.session:
...
def july_print_live_ob_ch8(request):
    if 'username' in request.session:
        ag = pg1_new_beds.objects.all().order_by('roon_no')
        l = []
        t = []
...
def auguest_details_live_ob_ch8(request):
    if 'username' in request.session:
...
def auguest_print_live_ob_ch8(request):
    if 'username' in request.session:
        ag = pg1_new_beds.objects.all().order_by('roon_no')
        l = []
        t = []
...
def sept_details_live_ob_ch8(request):
    if 'username' in request.session:
...
def sept_print_live_ob_ch8(request):
    if 'username' in request.session:
        ag = pg1_new_beds.objects.all().order_by('roon_no')
        l = []
        t = []
...
def october_details_live_ob_ch8(request):
    if 'username' in request.session:
...
def october_print_live_ob_ch8(request):
    if 'username' in request.session:
        ag = pg1_new_beds.objects.all().order_by('roon_no')
        l = []
        t = []
...
def nov_details_live_ob_ch8(request):
    if 'username' in request.session:
...
def nov_print_live_ob_ch8(request):
    if 'username' in request.session:
        ag = pg1_new_beds.objects.all().order_by('roon_no')
        l = []
        t = []
...
def dec_details_live_ob_ch8(request):
    if 'username' in request.session:
...
def dec_print_live_ob_ch8(request):
    if 'username' in request.session:
        ag = pg1_new_beds.objects.all().order_by('roon_no')
        l = []
        t = []
...
def viewall_vaccant_room_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def full_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def jan_full_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def feb_full_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def march_full_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def april_full_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def may_full_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def june_full_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def july_full_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def auguest_full_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def sept_full_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def october_full_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def nov_full_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def dec_full_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def partially_paid_guest_choose_months_ob_ch8(request):
    if 'username' in request.session:
...
def jan_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def table_jan_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def feb_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def table_feb_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def march_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def table_march_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def april_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def table_april_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def may_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def table_may_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def june_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def table_june_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def july_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def table_july_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def auguest_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def table_auguest_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def sept_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def table_sept_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def october_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def table_october_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def nov_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def table_nov_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def dec_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def table_dec_partially_paid_guest_ob_ch8(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
