...
def monthly_details_due_ob_ch81(request):
    jan_tc = []
    total_guest_br10 = branch10app.models.pg1_new_guest.objects.all().filter(flag=2, jan_rent_flag__gt=99)
    for i in total_guest_br10:
        if i.monthly_rent != '':
...
# ****grand_total_collection start here
    ...
    context={
        ...
        'vcated_guest_all_details': vacate_guest_calculations81.monthly_collection_details_vcated(),
    }

    return render(request,'branches/branch10/reports/dashboard_reports/due_amount/monthly_details.html',context)

def monthly_collection_details_ob_ch81(request):
    jan_tc = []
    total_guest_br10 = branch10app.models.pg1_new_guest.objects.all().filter(flag=2, jan_rent_flag__gt=99)
    for i in total_guest_br10:
        if i.monthly_rent != '':
